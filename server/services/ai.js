...
async function analyzeFoodImage(imageBuffer, mimeType, userMemo, recentMeals, targets, mealType, foodSource, clarify) {
  ...
  const sourceBlock = foodSource === 'conbini'
    ...
    ? '\n## この食事は【外食】です\n'
      ...
      + 'improve と try は必ず「**次の注文で選べること**」にする: 単品より定食(主食+主菜+副菜)、揚げ物より焼き魚/蒸し料理/生姜焼き、'
      + 'ご飯は少なめ/半ライス、麺類は汁を残す、+サラダや小鉢で野菜、ドレッシング/ソースは別添え。\n'
      + '- **同じ店で頼める範囲**の提案にする。良い選択ができている時はそう認める。\n'
      + '- 「我慢ではなく選び方で整う」と、外食を楽しみながら実践できる一言で締める。\n'
    : '';
